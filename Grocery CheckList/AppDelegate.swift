...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    // MARK: -
    // MARK: Helper Methods
    fileprivate func seedItems() {
        let ud = UserDefaults.standard
       
        if !ud.bool(forKey: "UserDefaultsSeedItems") {
        
            let filePath = Bundle.main.path(forResource: "seed", ofType: "plist")
            let seedItems = NSArray(contentsOfFile: filePath!) as! [[String:AnyObject]]
            // Items
            var items = [Item]()
            ...
            for seedItem in seedItems {
                if let name = seedItem["name"] as? String, let price = seedItem["price"] as? Float {
                    ...
                    print("\(String(describing: name)) - \(String(describing: price))")
                    ...
                    // Add Item
                    items.append(item)
                    NotificationCenter.default.post(name: NSNotification.Name(rawValue: "load"), object: items)
                }
